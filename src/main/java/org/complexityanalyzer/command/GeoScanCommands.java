...
public class GeoScanCommands {
    ...
    private static int showProfileHelp(CommandContext<CommandSourceStack> context) {
        ...
        output.sendInfo(source, Component.literal("  💻 Your system: ")
        ...
        for (ScanProfile profile : ScanProfile.values()) {
            ...
            MutableComponent profileLine = Component.literal("  " + icon + " ")
                    .append(Component.literal(profile.name().toLowerCase())
                            .withStyle(color, ChatFormatting.BOLD))
                    .append(Component.literal(" — ")
                            .withStyle(ChatFormatting.DARK_GRAY));

            profileLine.append(Component.literal(workers + " workers")
                    .withStyle(ChatFormatting.WHITE));
            profileLine.append(Component.literal(" (" + percent + "% CPU)")
                    .withStyle(ChatFormatting.GRAY));
            ...
            if (profile.hasMsptLimit()) {
                ...
                profileLine.append(Component.literal(" | NO LIMIT")
            ...
            tooltipLines.add(Component.literal("Workers: ")
                    .withStyle(ChatFormatting.GRAY)
                    .append(Component.literal(workers + "/" + totalThreads)
                            .withStyle(ChatFormatting.WHITE)));
            tooltipLines.add(Component.literal("CPU Usage: ")
                    .withStyle(ChatFormatting.GRAY)
                    .append(Component.literal(percent + "%")
                            .withStyle(ChatFormatting.WHITE)));
            tooltipLines.add(Component.literal("Max Parallel Chunks: ")
                    .withStyle(ChatFormatting.GRAY)
                    .append(Component.literal(String.valueOf(profile.maxParallelChunks))
                            .withStyle(ChatFormatting.WHITE)));

            if (profile.hasMsptLimit()) {
                tooltipLines.add(Component.literal(""));
                tooltipLines.add(Component.literal("⚡ Auto-throttle enabled")
                        .withStyle(ChatFormatting.AQUA));
                tooltipLines.add(Component.literal("Pauses when MSPT > " + (int) profile.msptLimit + "ms")
                        .withStyle(ChatFormatting.GRAY));
                tooltipLines.add(Component.literal("Resumes when MSPT < " +
                                (int) (profile.msptLimit - ScanConfig.MSPT_RECOVERY_THRESHOLD_MS) + "ms")
                        .withStyle(ChatFormatting.GRAY));
            } else {
                tooltipLines.add(Component.literal(""));
                tooltipLines.add(Component.literal("⚠ No throttle protection!")
                        .withStyle(ChatFormatting.RED));
                tooltipLines.add(Component.literal("May cause severe lag")
        ...
        output.sendInfo(source, Component.literal("  📝 Usage: ")
                .withStyle(ChatFormatting.GRAY)
                .append(Component.literal("/complexity geoscan start <profile> [chunks] [force]")
                        .withStyle(ChatFormatting.YELLOW)));

        output.sendInfo(source, Component.literal("  📝 Example: ")
                .withStyle(ChatFormatting.GRAY)
    ...
    private static int executeScan(CommandContext<CommandSourceStack> context,
        ...
        try {
            ...
            output.sendInfo(source,
                    Component.literal("Use ")
                            .withStyle(ChatFormatting.GRAY)
                            .append(Component.literal("/complexity geoscan start")
                                    .withStyle(ChatFormatting.YELLOW))
                            .append(Component.literal(" to see available profiles")
                                    .withStyle(ChatFormatting.GRAY)));
            return 0;
        }
        ...
        AnalysisEngine.getInstance().getGeoManager().ifPresentOrElse(
                manager -> {
                    ...
                    output.sendInfo(source, Component.literal("  Profile: ")
                            .withStyle(ChatFormatting.GRAY)
                            .append(Component.literal(icon + " " + profile.name())
                                    .withStyle(color, ChatFormatting.BOLD)));

                    output.sendInfo(source, Component.literal("  Workers: ")
                            .withStyle(ChatFormatting.GRAY)
                            .append(Component.literal(workers + "/" + totalThreads + " threads")
                                    .withStyle(ChatFormatting.WHITE)));

                    output.sendInfo(source, Component.literal("  Chunks/biome: ")
                            .withStyle(ChatFormatting.GRAY)
                            .append(Component.literal(String.valueOf(chunks))
                                    .withStyle(ChatFormatting.AQUA)));

                    if (profile.hasMsptLimit()) {
                        output.sendInfo(source, Component.literal("  Protection: ")
                                .withStyle(ChatFormatting.GRAY)
                    ...
                    output.sendInfo(source, Component.literal("  Initiator: ")
                            .withStyle(ChatFormatting.GRAY)
                    ...
                    output.sendToAdmins(Component.literal("[GeoScan] " +
                            (force ? "Force started" : "Scheduled") + " by " + initiatorName +
                            " | Profile: " + profile.name() + " | Chunks: " + chunks));
                },
                () -> output.sendFailure(source,
    ...
    private static int executeStop(CommandContext<CommandSourceStack> context) {
        ...
        AnalysisEngine.getInstance().getGeoManager().ifPresentOrElse(
                manager -> {
                    ...
                    output.broadcast(Component.literal("✓ Geo-scan stopped")
                            .withStyle(ChatFormatting.GREEN));
        ...
        AnalysisEngine.getInstance().getGeoManager().ifPresentOrElse(
                manager -> {
                    ...
                    if (session != null && session.isValid()) {
                        ...
                        output.sendInfo(source, Component.literal("  " + icon + " ")
                                .withStyle(color)
                                .append(Component.literal(status)
                                        .withStyle(color)));
                    }
    ...
    private static void renderActiveStatus(OutputManager output, CommandSourceStack source,
        ...
        output.sendInfo(source, Component.literal("  Workers: ")
                .withStyle(ChatFormatting.GRAY)
                .append(Component.literal(activeWorkers + "/" + targetWorkers)
                        .withStyle(ChatFormatting.WHITE))
        ...
        output.sendInfo(source, Component.literal("  Progress: ")
                .withStyle(ChatFormatting.GRAY)
                .append(Component.literal(progressBar)
                        .withStyle(percent >= 100 ? ChatFormatting.GREEN : ChatFormatting.AQUA)));

        output.sendInfo(source, Component.literal("  Chunks: ")
                .withStyle(ChatFormatting.GRAY)
                .append(Component.literal(scanned + "/" + total)
                        .withStyle(ChatFormatting.WHITE)));

        float speed = session.getScanSpeed();
        output.sendInfo(source, Component.literal("  Speed: ")
                .withStyle(ChatFormatting.GRAY)
        ...
        output.sendInfo(source, Component.literal("  Elapsed: ")
                .withStyle(ChatFormatting.GRAY)
        ...
        if (profile.hasMsptLimit()) {
            ...
            MutableComponent msptLine = Component.literal("  MSPT: ")
                    .withStyle(ChatFormatting.GRAY);

            if (throttled) {
                msptLine.append(Component.literal(String.format("%.1f", mspt))
                                .withStyle(ChatFormatting.RED))
        ...
        biomeStatus.append(Component.literal("[hover for details]")
    ...
    private static MutableComponent buildBiomeTooltip(ScanSession session) {
        ...
        for (var dimEntry : progress.entrySet()) {
            ...
            tooltip.append(Component.literal("\n")
                    .append(Component.literal("▸ " + formatDimensionName(dimId) + "\n")
                            .withStyle(ChatFormatting.YELLOW, ChatFormatting.BOLD)));

            for (var biomeEntry : dimEntry.getValue().entrySet()) {
                if (biomesShown >= maxBiomes) {
                    int remaining = 0;
                    for (var de : progress.values()) {
                        remaining += de.size();
                    }
                    remaining -= maxBiomes;
                    if (remaining > 0) {
                ...
                if (complete) {
                    biomeLine.append(Component.literal("✓ ")
                            .withStyle(ChatFormatting.GREEN));
                    biomeLine.append(Component.literal(biomeId.getPath())
                            .withStyle(ChatFormatting.GREEN));
