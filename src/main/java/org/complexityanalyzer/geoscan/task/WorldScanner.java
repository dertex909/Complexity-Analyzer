...
public class WorldScanner {
    ...
    private Optional<BlockPos> locateBiome(ServerLevel level, ResourceKey<Biome> biomeKey, BlockPos origin, int radius) {
        ...
        ComplexityAnalyzer.LOGGER.debug("[WorldScanner] Searching for {} from [{}, {}, {}], radius={}",
                biomeKey.location(), origin.getX(), origin.getY(), origin.getZ(), radius);

        try {
            BlockPos result = FastBiomeFinder.findBiome(
                    level,
            ...
            if (result == null && radius > 10000) {
                ComplexityAnalyzer.LOGGER.debug("[WorldScanner] Spiral search failed, trying random search...");
                result = FastBiomeFinder.randomSearch(
                        level,
                        holder -> holder.is(biomeKey),
                        origin,
                        radius,
            ...
            if (result != null) {
                ComplexityAnalyzer.LOGGER.debug("[WorldScanner] ✓ Found {} at [{}, {}, {}]",
                        biomeKey.location(), result.getX(), result.getY(), result.getZ());
            } else {
            ...
            if (!shouldStop()) {
                ComplexityAnalyzer.LOGGER.error("[WorldScanner] Error locating biome {}: {}",
                        biomeKey.location(), e.getMessage(), e);
            }
            return Optional.empty();
        }
    }
