...
export const ITEM_FLAG = {
    HAS_RECIPE: 0x01, IS_HARDCODED: 0x04, IS_VALID: 0x08, IS_UNCALCULABLE: 0x10, NO_RECIPE: 0x20,
};

export const FLUID_FLAG = {
    HAS_RECIPE: 0x01, IS_VALID: 0x08, IS_UNCALCULABLE: 0x10, NO_RECIPE: 0x20, IS_PROTECTED: 0x40,
};
...
export function readMachineIndex(bytes) {
    const b = new Buf(bytes);
    const count = b.i32();
    const machines = [];
    let payloadStart = b.p + count * 10;
    for (let i = 0; i < count; i++) {
        const itemIndex = b.i32();
        const offset = b.i32();
...
export function readSourceTypeIndex(bytes) {
    const b = new Buf(bytes);
    const count = b.i32();
    const types = [];
    for (let i = 0; i < count; i++) {
...
export function readModSummary(bytes, strings) {
    const b = new Buf(bytes);
    const count = b.i32();
    const mods = [];
    for (let i = 0; i < count; i++) {
